...
export function ActivityForm({
  ...
  const [subType, setSubType] = useState(ACTIVITY_SUB_TYPES.transport[0]);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setQuantity('');
      setSubType(ACTIVITY_SUB_TYPES[category][0]);
      onSuccess?.();
    } catch (err) {
  ...
  return <Card className="w-full max-w-2xl mx-auto border-emerald-100/50 dark:border-emerald-900/50 shadow-md">
      <CardHeader>
          ...
          <Tabs value={category} onValueChange={handleCategoryChange} className="w-full">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <select id="subType" value={subType} onChange={e => setSubType(e.target.value)} className="flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50" required>
                  {ACTIVITY_SUB_TYPES[category].map(type => <option key={type} value={type}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input id="quantity" type="number" min="0" step="0.1" placeholder="e.g. 15.5" value={quantity} onChange={e => setQuantity(e.target.value)} required className="pr-16" />
