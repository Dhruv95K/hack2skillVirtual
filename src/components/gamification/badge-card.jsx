...
export function BadgeCard({
  ...
  return <motion.div initial={earned && !shouldReduceMotion ? {
    scale: 0.9,
    opacity: 0
  } : false} animate={earned && !shouldReduceMotion ? {
    scale: 1,
    opacity: 1
  } : false} transition={{
    type: 'spring',
        ...
        <CardContent className="p-6 flex flex-col items-center text-center gap-3">
          ...
          <div className="mt-auto pt-4 text-xs font-medium">
            {earned ? <span className="text-green-600">
                Earned {earnedAt ? new Date(earnedAt).toLocaleDateString('en-US', {
              month: 'short',
              day: 'numeric',
              year: 'numeric'
            }) : ''}
              </span> : <span className="flex items-center gap-1 justify-center text-muted-foreground">
