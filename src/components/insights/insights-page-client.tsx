  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      {Array.from({ length: SKELETON_COUNT }).map((_, index) => (
        <Card key={index} className="border-l-4 border-accent bg-surface/95 ring-white/8">
  ...
  const loadSavedInsights = useCallback(async () => {
    try {
      ...
      setMessage(
        (data.tips ?? []).length > 0
          ? null
          : data.message ?? 'Generate personalized tips from your latest activity logs.'
      );
    } catch (error) {
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col items-start gap-3 lg:items-end">
            <Button
              ...
              className="cursor-pointer bg-emerald-600 px-4 text-white hover:bg-emerald-500"
            >
            ...
            <div
              aria-live="polite"
              className="flex min-h-5 items-center gap-2 text-xs text-slate-400"
            >
            ...
            {isBootstrapping || isGenerating ? (
              ...
              >
                {tips.slice(0, 5).map((tip) => (
                  <InsightCard key={`${tip.category}-${tip.title}`} {...tip} />
                ))}
              </motion.div>
            ) : (
              <div className="rounded-2xl border border-dashed border-white/10 bg-white/3 px-5 py-10 text-center">
