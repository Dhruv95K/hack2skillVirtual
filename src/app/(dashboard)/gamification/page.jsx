...
export default function GamificationPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/gamification').then(res => {
  ...
  return <div className="max-w-4xl mx-auto space-y-8 p-4 md:p-8">
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2 p-6 rounded-xl border bg-card text-card-foreground shadow-sm">
          <LevelProgress level={data.level} levelName={data.levelName} co2Saved={data.totalCo2Saved} nextLevelThreshold={data.nextLevelThreshold} nextLevelName={data.nextLevelName} currentLevelThreshold={data.currentLevelThreshold} />
        </div>
      </div>
      ...
      <div className="space-y-4 pt-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          ...
          return <BadgeCard key={badge.id} badge={badge} earned={isEarned} earnedAt={earnedRecord?.earnedAt} />;
        })}
        </div>
      </div>
    </div>;
