...
const VALID_CATEGORIES = ['transport', 'food', 'energy'] as const;
...
function isValidCategory(category: unknown): category is AiTip['category'] {
  return typeof category === 'string' && VALID_CATEGORIES.includes(category as AiTip['category']);
}

function roundSaving(value: number) {
  return Math.round(value * 10) / 10;
}

function normaliseTip(rawTip: unknown): AiTip | null {
  if (!rawTip || typeof rawTip !== 'object') {
    return null;
  }

  const candidate = rawTip as Record<string, unknown>;
...
function parseTipsFromResponse(responseText: string): AiTip[] {
...
function buildUserSummary(logs: ActivityLogSummary[]) {
  const totals = logs.reduce<Record<AiTip['category'], number>>(
    (accumulator, log) => {
  ...
  const categoriesByImpact = Object.entries(totals).sort(([, a], [, b]) => b - a) as Array<
    [AiTip['category'], number]
  >;
  ...
  return {
    topCategory,
    prompt: [
      `Top-emitting category: ${topCategory}`,
      `Category totals: ${categoriesByImpact
        .map(([category, total]) => `${category}=${total.toFixed(1)} kg`)
        .join(', ')}`,
...
export async function GET(request: Request) {
  ...
  try {
      ...
      return NextResponse.json({
        ...
        message:
          recentLogs.length === 0
            ...
            : 'Generate personalized tips from your latest activity logs.',
      });
    }
    ...
    return NextResponse.json({ error: 'Unable to load insights right now.' }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { prompt, topCategory } = buildUserSummary(recentLogs);
