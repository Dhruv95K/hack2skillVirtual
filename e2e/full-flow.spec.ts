...
test.describe('EcoTrack Full E2E Journey', () => {
  test('User can sign up, take quiz, log activity, and view insights', async ({ page, context }) => {
    ...
    await page.route('/api/auth/signup', async (route) => {
      ...
      if (request.method() === 'POST') {
        ...
        if (body.email && body.password.length >= 8) {
          await context.addCookies([{ name: 'e2e-mock-auth', value: 'true', domain: 'localhost', path: '/' }]);
          await route.fulfill({ status: 201, json: { user: { id: 'mock-id', email: body.email } } });
        } else {
          await route.fulfill({ status: 400, json: { error: 'Invalid data' } });
        }
      } else {
    ...
    // 1. Landing Page
    await page.goto('/');
    await expect(page.getByRole('heading', { name: /Carbon Footprint/i }).first()).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: /your carbon footprint quiz/i })).toBeVisible();
    ...
    // 5. Log Activity
    await page.getByRole('link', { name: /Log Activity/i }).first().click();
    ...
    await expect(page.getByText(/Fresh AI insights generated/i)).toBeVisible({ timeout: 15000 });

    // 7. Gamification
    await page.getByRole('link', { name: /Gamification/i }).first().click();
    ...
    // 8. Offsets
    await page.getByRole('link', { name: /Offsets/i }).first().click();
    await expect(page).toHaveURL(/.*\/offsets/);
    await expect(page.getByRole('heading', { name: /Support Carbon Offset Programs/i })).toBeVisible();
