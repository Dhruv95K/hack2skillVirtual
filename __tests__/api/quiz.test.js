...
describe('POST /api/quiz', () => {
  ...
  it('returns 409 if the user already completed the quiz', async () => {
    ...
    prisma.quizResponse.count.mockResolvedValue(2);
    const request = new NextRequest('http://localhost/api/quiz', {
      method: 'POST',
      body: JSON.stringify({
        responses: {
  ...
  it('returns 409 if a concurrent submission loses to the unique write constraint', async () => {
    ...
    prisma.quizResponse.count.mockResolvedValue(0);
    calculateBaseline.mockReturnValue({
      total: 1000,
      transport: 400,
      food: 300,
      energy: 300
  ...
  it('uses the real route in e2e mode and marks quiz completion via cookie', async () => {
    ...
    calculateBaseline.mockReturnValue(mockBaseline);
    const request = new NextRequest('http://localhost/api/quiz', {
      method: 'POST',
      headers: {
  ...
  it('saves responses, updates user baseline, and returns success', async () => {
    ...
    prisma.$transaction.mockImplementation(async callback => callback({
      quizResponse: {
        createMany: transactionQuizCreateMany
      },
      user: {
    ...
    expect(transactionQuizCreateMany).toHaveBeenCalledWith({
      data: [{
        userId: 'user-1',
        category: 'transport',
        questionKey: 'primary_transport',
        answer: 'car_petrol'
      }, {
        userId: 'user-1',
        category: 'transport',
        questionKey: 'weekly_km',
        answer: '100'
      }, {
        userId: 'user-1',
        category: 'transport',
        questionKey: 'flights_per_year',
        answer: '1'
      }, {
        userId: 'user-1',
        category: 'food',
        ...
        userId: 'user-1',
        category: 'food',
        questionKey: 'meat_meals_per_week',
        answer: '0'
      }, {
        userId: 'user-1',
        category: 'energy',
        questionKey: 'home_size',
        answer: '2bedroom'
      }, {
        userId: 'user-1',
        category: 'energy',
        questionKey: 'monthly_electricity_kwh',
        answer: '200'
      }]
    });
