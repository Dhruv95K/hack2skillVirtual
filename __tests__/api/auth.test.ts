...
describe('Auth API Routes', () => {
  ...
  describe('POST /api/auth/signup', () => {
    ...
    it('returns 400 if email already exists', async () => {
      const mockSignUp = jest.fn().mockResolvedValue({ data: null, error: { message: 'User already registered' } });
      (createClient as jest.Mock).mockResolvedValue({ auth: { signUp: mockSignUp } });

      const request = new NextRequest('http://localhost/api/auth/signup', {
        method: 'POST',
        body: JSON.stringify({ email: 'existing@example.com', name: 'Existing User', password: 'password123' }),
      });
  ...
  describe('POST /api/auth/signin', () => {
    it('returns 200 with valid credentials', async () => {
      const mockSignIn = jest.fn().mockResolvedValue({ data: { user: { id: 'user-1', email: 'test@example.com' } }, error: null });
      (createClient as jest.Mock).mockResolvedValue({ auth: { signInWithPassword: mockSignIn } });
    ...
    it('returns 401 with wrong password', async () => {
      const mockSignIn = jest.fn().mockResolvedValue({ data: null, error: { message: 'Invalid login credentials' } });
      (createClient as jest.Mock).mockResolvedValue({ auth: { signInWithPassword: mockSignIn } });
