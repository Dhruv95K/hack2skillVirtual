...
describe('shouldAwardBadge', () => {
  ...
  it('handles transport_logs', () => { expect(shouldAwardBadge('green_commuter', { totalLogs: 10, streak: 0, transportLogs: 10, plantLogs: 0, energyLogs: 0, co2Saved: 0 })).toBe(true); });
  it('handles plant_logs', () => { expect(shouldAwardBadge('plant_power', { totalLogs: 10, streak: 0, transportLogs: 0, plantLogs: 10, energyLogs: 0, co2Saved: 0 })).toBe(true); });
  it('handles energy_logs', () => { expect(shouldAwardBadge('energy_saver', { totalLogs: 10, streak: 0, transportLogs: 0, plantLogs: 0, energyLogs: 10, co2Saved: 0 })).toBe(true); });
  it('handles co2_saved', () => { expect(shouldAwardBadge('carbon_crusher', { totalLogs: 10, streak: 0, transportLogs: 0, plantLogs: 0, energyLogs: 0, co2Saved: 100 })).toBe(true); });
  it('returns false for unknown badge key', () => { expect(shouldAwardBadge('unknown', { totalLogs: 10, streak: 0, transportLogs: 0, plantLogs: 0, energyLogs: 0, co2Saved: 100 })).toBe(false); });
});
...
describe('updateStreak', () => {
  ...
  it('sets streak to 1 if first log ever', async () => {
    (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'u1', streak: 0, lastLoggedAt: null });
    (prisma.user.update as jest.Mock).mockResolvedValue({ streak: 1 });
    const res = await updateStreak('u1');
    expect(prisma.user.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ streak: 1 }) }));
    expect(res).toEqual({ streak: 1 });
  });

  it('increments streak if logged yesterday', async () => {
    const yesterday = new Date(); yesterday.setUTCDate(yesterday.getUTCDate() - 1);
    (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'u1', streak: 1, lastLoggedAt: yesterday });
    (prisma.user.update as jest.Mock).mockResolvedValue({ streak: 2 });
    await updateStreak('u1');
    expect(prisma.user.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ streak: 2 }) }));
  });

  it('resets streak to 1 if gap is more than 1 day', async () => {
    const lastWeek = new Date(); lastWeek.setUTCDate(lastWeek.getUTCDate() - 5);
    (prisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'u1', streak: 10, lastLoggedAt: lastWeek });
    (prisma.user.update as jest.Mock).mockResolvedValue({ streak: 1 });
    await updateStreak('u1');
    expect(prisma.user.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ streak: 1 }) }));
  });
  ...
  it('does nothing and returns user if already logged today', async () => {
    ...
    const user = { id: 'u1', streak: 5, lastLoggedAt: today };
...
describe('checkAndAwardBadges', () => {
  it('awards badges and updates level', async () => {
    const mockUser = { id: 'u1', streak: 5, totalCo2Tracked: 100, level: 1 };
    (prisma.activityLog.count as jest.Mock).mockImplementation(({ where }) => {
      if (where.category) return Promise.resolve(0);
      return Promise.resolve(10); // totalLogs = 10
    });
    const allBadges = [
      { id: 'b1', key: 'first_step', thresholdType: 'total_logs', thresholdValue: 1 },
      { id: 'b2', key: 'streak_7', thresholdType: 'streak', thresholdValue: 7 }
    ];
    (prisma.badge.findMany as jest.Mock).mockResolvedValue(allBadges);
    (prisma.userBadge.findMany as jest.Mock).mockResolvedValue([]);
    
    await checkAndAwardBadges('u1', mockUser as any);
    ...
    expect(prisma.user.update).toHaveBeenCalledWith({
      where: { id: 'u1' }, data: { level: expect.any(Number) }
    });
  });

  it('does not award badges if already earned', async () => {
    const mockUser = { id: 'u1', streak: 5, totalCo2Tracked: 100, level: 7 }; // level 7 matches 100kg
    (prisma.activityLog.count as jest.Mock).mockResolvedValue(10);
    const allBadges = [
      { id: 'b1', key: 'first_step', thresholdType: 'total_logs', thresholdValue: 1 }
    ];
    (prisma.badge.findMany as jest.Mock).mockResolvedValue(allBadges);
    (prisma.userBadge.findMany as jest.Mock).mockResolvedValue([{ badgeId: 'b1' }]); // already has it
